...
def _chunk_by_num_ticks(table: pa.Table, start_tick: int, end_tick: int) -> List[Tuple[int, int, Optional[int], Optional[int]]]:
    ...
    # Stores the (first_tick, last_tick, first_row, last_row) for each tick chunk. If no data is present for a tick range, first_row and last_row will be None.
    tick_chunks = []
    cur_chunk_start_row = 0
    cur_first_tick = start_tick
    # The tick which will cut this one off at
    cur_last_tick = min(start_tick - 1 + TICKS_PER_CHUNK, end_tick)
    ...
    for i in range(total_num_rows):
        ...
        while tick >= cur_last_tick:
            if i == cur_chunk_start_row:  # This tick chunk has no data, so we append an empty tuple
                ...
                tick_chunks.append(
                    (cur_first_tick, cur_last_tick, cur_chunk_start_row, i - 1))
    ...
    while cur_last_tick <= end_tick:
        if cur_chunk_start_row < total_num_rows - 1:  # We still have some data in our current chunk
            tick_chunks.append(
                (cur_first_tick, cur_last_tick, cur_chunk_start_row, total_num_rows - 1))
...
async def stream_datasets(demo_id: str, dataset_names: List[str] = Query(...), round_num: Optional[int] = None) -> StreamingResponse:
    ...
    """
    logger.info(
        f"stream_demo_datasets(dataset_names={dataset_names}, demo_id={demo_id}, round_num={round_num})")
    ...
    try:
        ...
        # Maps from dataset name to their schemas to avoid having to fetch schemas repeatedly
        dataset_schemas = {}
        for dataset in dataset_names:
            # Load the Pandas DF
            demo_dataset = demo_coach_manager.get_demo_data(
                dataset, demo_id, round_num)
            ...
            dataset_chunks[dataset] = _chunk_by_num_ticks(
                demo_dataset_table, start_tick, end_tick)
        ...
        for dataset_chunk in dataset_chunks.values():
            if num_chunks == -1:
                ...
                raise ValueError(
                    "Datasets have different amount of chunks when they are expected to have the same amount.")
        ...
        async def generate_data():
            for chunk_idx in range(num_chunks):
                ...
                for dataset_name in dataset_names:
                    first_tick, last_tick, first_row, last_row = dataset_chunks[
                        dataset_name][chunk_idx]
                    ...
                    if first_row is None or last_row is None:
                        ...
                        # Slices based on length
                        cur_chunk = table.slice(
